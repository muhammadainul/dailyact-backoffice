...
jQuery(document).ready( function (){
    jQuery.getJSON( "/assets/js/lib/amcharts/tools-country-id.json", function( geo ) {
      var chartStat = $.ajax({
          method: 'POST',
          url: '/bizdev/demographyByPostImageFile',
          dataType: 'json',
          crossDomain: true,
          headers: {
          ...
          success: function (results) {
            ...
            // Create map instance
            var chart = am4core.create("chartdivimagefile", am4maps.MapChart);
            ...
            polygonSeries.data = results.provinceFileGroup
            ...
            for (let i of polygonSeries.data) {
                ...
                console.log("kjlj"+locationName);
                jQuery("#tableStatLocImageFile").append(`<tr>><td style='width: 5%;'>` + num + `</td><td>` + locationName + `</td>
                    <td>` + i.value + `</td></tr>`);
                // if (count < 34) {
                //   jQuery("#tableStatLocImageFile").append(`<tr>><td style='width: 5%;'>` + num + `</td><td>` + locationName[count] + `</td>
                //   <td>` + i.value + `</td></tr>`);
                //   count++;
                // } 
            }
            jQuery("#tableStatLocImageFile").append("<tr><td>&nbsp</td></td><td><b>TOTAL</b></td><td><b>" + totalValue + "</b></td></tr>");
        }
    });
  });
});
...
function testFunc(param) {
    ...
    jQuery("#viewContentPostImageFile").modal("show");
      // var myVal = $(this).attr('data-id');
      jQuery("#postImageFile").html("<center>" + param + "</center>");
    // });
    //   }
    // });
}         