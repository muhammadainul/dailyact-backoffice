...
jQuery(document).ready( function (){
    jQuery.getJSON( "/assets/js/lib/amcharts/tools-country-id.json", function( geo ) {
      var chartStat = $.ajax({
          method: 'POST',
          url: '/bizdev/demographyByLastActive',
          dataType: 'json',
          crossDomain: true,
          headers: {
          ...
          success: function (results) {
            ...
            // Create map instance
            var chart = am4core.create("chartdivlastactive", am4maps.MapChart);
            ...
            for (let i of polygonSeries.data) {
                ...
                jQuery("#tableStatLoc").append(`<tr>><td style='width: 5%;'>` + num + `</td><td>` + locationName + `</td>
                    <td>` + i.value + `</td></tr>`);
            }
            jQuery("#tableStatLoc").append("<tr><td>&nbsp</td></td><td><b>TOTAL</b></td><td><b>" + totalValue + "</b></td></tr>");
        }
    });
  });
});
...
function testFunc(param) {
    ...
    jQuery("#viewContentLastActive").modal("show");
      // var myVal = $(this).attr('data-id');
      jQuery("#lastActiveData").html("<center>" + param + "</center>");
    // });
    //   }
    // });
}         