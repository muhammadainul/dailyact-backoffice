...
jQuery(document).ready( function (){
    jQuery.getJSON( "/assets/js/lib/amcharts/tools-country-id.json", function( geo ) {
      var chartStat = $.ajax({
          method: 'POST',
          url: '/bizdev/demographyByStoryData',
          dataType: 'json',
          crossDomain: true,
          headers: {
          ...
          success: function (results) {
            ...
            // Create map instance
            var chart = am4core.create("chartdivstory", am4maps.MapChart);
            ...
            for (let i of polygonSeries.data) {
                ...
                jQuery("#tableStatLocStory").append(`<tr>><td style='width: 5%;'>` + num + `</td><td>` + locationName + `</td>
                    <td>` + i.value + `</td></tr>`);
            }
            jQuery("#tableStatLocStory").append("<tr><td>&nbsp</td></td><td><b>TOTAL</b></td><td><b>" + totalValue + "</b></td></tr>");
        }
    });
  });
});
...
function testFunc(param) {
    ...
    jQuery("#viewContentStoryData").modal("show");
      // var myVal = $(this).attr('data-id');
      jQuery("#storyImageStoryData").html("<center>" + param + "</center>");
    // });
    //   }
    // });
}         